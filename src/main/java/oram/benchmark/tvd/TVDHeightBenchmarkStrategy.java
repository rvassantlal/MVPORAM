...
public class TVDHeightBenchmarkStrategy implements IBenchmarkStrategy, IWorkerStatusListener {
	...
	public void executeBenchmark(WorkerHandler[] workerHandlers, Properties benchmarkParameters) {
		...
		for (int treeHeight : treeHeights) {
			BigDecimal[][] tvds = new BigDecimal[nAccessThresholds][];
			for (int i = 0; i < nAccessThresholds; i++) {
				...
				tvds[i] = tvdClient.calculateHeightTVD(clientsPerRound,
						treeHeight, zipfParameter, threshold);

			}
			String fileName = String.format("tvd_heights_height_%d.dat", treeHeight);
			storeTVD(tvds, zipfParametersStr, accessThresholdsStr, clientsPerRound, fileName);
		}
	}

	private void storeTVD(BigDecimal[][] tvd, String[] zipfianParameters, String[] accessThresholds, int[] clientsPerRound, String filename) {
		...
		try (BufferedWriter resultFile = new BufferedWriter(new OutputStreamWriter(Files.newOutputStream(path)))) {
			...
			for (int i = 0; i < zipfianParameters.length; i++) {
				resultFile.write(String.format("\tzipf=%s,threshold=%s", zipfianParameters[i], accessThresholds[i]));
			}
			resultFile.newLine();
			...
			for (int i = 0; i < clientsPerRound.length; i++) {
				...
				for (int j = 0; j < zipfianParameters.length; j++) {
